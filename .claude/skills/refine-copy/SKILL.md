---
name: refine-copy
description: "Remove AI writing patterns from text: significance inflation, AI vocabulary, formulaic structure, hedging, rhythmic monotony, and soulless prose. Use when content sounds like it was AI-generated. Rewrites or reviews in-place. Called automatically by all export skills."
argument-hint: "[file-path | --review-only]"
user-invocable: true
allowed-tools:
  - Read
  - Write
...
## Phase 0: Input (skip when called by another skill with a file already set)

If invoked directly by the user with no file context:

**Step 1 — Source:**
Use `AskUserQuestion`:
- Prompt: "What should I refine?"
- Options:
  - `A file — I'll give you the path`
  - `Text I'll paste directly`

If file: ask for the path, then read it.
If paste: ask the user to paste the content into the next message.

**Step 2 — Mode:**
Use `AskUserQuestion`:
- Prompt: "What do you want?"
- Options:
  - `Rewrite — apply all passes, give me the revised version`
  - `Review only — flag AI patterns without rewriting`

Store mode as `[MODE]`. Proceed to The Editing Process with this context.
...
Certain words and phrases are dead giveaways.
...
- When called directly by the user: never overwrite a file without asking first (Phase 0 establishes the source and mode)
- When called by an export skill: edit the file in-place without an approval gate — the export skill's own approval gate already covered this
- Do not add the Changes table to saved files — only include it in the conversation reply
- Preserve technical accuracy: never change a specific fact, stat, date, or proper noun
- Do not apply Pass 8 (Soul) when the piece is formal documentation, patch notes, or legal/compliance text — voice is for marketing and editorial content only
